...
const state = { range:'7d',project:'all',task:'all',model:'all',start:initialStart,end:initialEnd,search:'',descending:true,page:0,leftMetric:'burn',language:'en' };
let filteredRecords = [];
let previousRecords = [];
...
function renderHeatmap(){const values=PROJECTS.flatMap(project=>MODELS.map(model=>({project,model,total:sum(filteredRecords.filter(r=>r.project===project&&r.model===model),'total')}))),max=Math.max(...values.map(v=>v.total),1);let html='<div></div>';MODELS.forEach(model=>html+=`<div class="heatmap-label heatmap-column">${escapeHTML(shortModel(model))}</div>`);PROJECTS.forEach(project=>{html+=`<div class="heatmap-label">${escapeHTML(project)}</div>`;MODELS.forEach(model=>{const value=values.find(v=>v.project===project&&v.model===model),intensity=value.total?12+value.total/max*68:0;html+=`<button class="heat-cell" data-project="${escapeHTML(project)}" style="background:${value.total?`color-mix(in srgb,var(--blue) ${intensity.toFixed(1)}%,var(--surface-soft))`:'var(--surface-raised)'}" title="${escapeHTML(project)} · ${escapeHTML(model)} · ${formatNumber(value.total)} tokens">${value.total?formatTokens(value.total):'—'}</button>`;});});$('model-project-heatmap').innerHTML=html;$('model-project-heatmap').querySelectorAll('[data-project]').forEach(button=>button.addEventListener('click',()=>{state.project=button.dataset.project;state.task='all';state.page=0;$('project-filter').value=state.project;populateTaskOptions();render();}));}
...
function renderOutliers(tasks){
  ...
  if(!tasks.length)svg+=`<text class="outlier-axis" x="270" y="125" text-anchor="middle">${t('noData')}</text>`;const threshold=percentile(tasks.map(t=>t.total),.9),labeled=tasks.filter(t=>t.total>=threshold).sort((a,b)=>b.total-a.total).slice(0,3).map(t=>t.taskId);
  tasks.forEach(task=>{const x=left+task.total/max*width,y=bottom-Math.max(0,Math.min(1,(task.cacheRate-minCache)/(maxCache-minCache)))*height,radius=4+Math.sqrt(task.count)*.75;svg+=`<g class="outlier-point" data-task="${task.taskId}" tabindex="0" role="button"><circle style="fill:${modelColor(task.model)}99;stroke:${modelColor(task.model)}" cx="${x}" cy="${y}" r="${radius}"/></g>`;if(labeled.includes(task.taskId))svg+=`<text class="outlier-label" x="${x+radius+4}" y="${y+3}">${escapeHTML(task.task.length>8?`${task.task.slice(0,8)}…`:task.task)}</text>`;});$('outlier-chart').innerHTML=svg;
  $('outlier-chart').querySelectorAll('[data-task]').forEach(element=>{const task=tasks.find(t=>t.taskId===element.dataset.task),open=()=>openTask(task.taskId);element.addEventListener('click',open);element.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();open();}});element.addEventListener('pointermove',event=>{const frame=$('outlier-chart').closest('.outlier-frame').getBoundingClientRect(),tip=$('outlier-tooltip');tip.innerHTML=`<strong>${escapeHTML(task.task)}</strong><br>${escapeHTML(task.project)} · ${escapeHTML(shortModel(task.model))}<br>${formatTokens(task.total)} tokens · ${formatTokens(task.tokensPerTurn)}/turn<br>Cache ${formatPercent(task.cacheRate*100)}`;tip.style.display='block';tip.style.left=`${Math.min(event.clientX-frame.left+12,frame.width-210)}px`;tip.style.top=`${Math.max(0,event.clientY-frame.top-78)}px`;});element.addEventListener('pointerleave',()=>{$('outlier-tooltip').style.display='none';});});
}
function renderProjects(){const previousMap=new Map(PROJECTS.map(project=>[project,sum(previousRecords.filter(r=>r.project===project),'total')])),values=PROJECTS.map(project=>({project,total:sum(filteredRecords.filter(r=>r.project===project),'total')})).filter(v=>v.total>0||state.project==='all').sort((a,b)=>b.total-a.total),grand=values.reduce((t,i)=>t+i.total,0);$('project-count').textContent=t('projects',values.filter(i=>i.total>0).length);$('project-list').innerHTML=values.length?values.map(item=>{const change=changeRate(item.total,previousMap.get(item.project)||0);return `<button class="project-item ${state.project===item.project?'selected':''}" data-project="${escapeHTML(item.project)}"><div class="project-line"><span class="project-name">${escapeHTML(item.project)}</span><strong class="project-value">${formatTokens(item.total)}</strong></div><div class="project-sub"><span>${grand?formatPercent(item.total/grand*100):'0%'}</span>${formatChange(change,'')}</div><div class="project-meter"><i style="--share:${grand?item.total/grand:0}"></i></div></button>`;}).join(''):`<div class="empty-state">${t('noProjectData')}</div>`;$('project-list').querySelectorAll('[data-project]').forEach(button=>button.addEventListener('click',()=>{state.project=state.project===button.dataset.project?'all':button.dataset.project;state.task='all';state.page=0;$('project-filter').value=state.project;populateTaskOptions();render();}));}
function renderTopTasks(tasks){const top=[...tasks].sort((a,b)=>b.total-a.total).slice(0,5),topThree=sum(top.slice(0,3),'total');$('top-task-summary').textContent=top.length?t('topShare',formatPercent(currentMetrics.total?topThree/currentMetrics.total*100:0)):t('noData');$('top-expensive-tasks').innerHTML=top.length?top.map((task,index)=>`<button class="expensive-item" data-task="${task.taskId}"><span class="expensive-index">${String(index+1).padStart(2,'0')}</span><span class="expensive-copy"><strong>${escapeHTML(task.task)}</strong><small>${escapeHTML(task.project)} · ${escapeHTML(shortModel(task.model))}</small></span><span class="expensive-value"><strong>${formatTokens(task.total)}</strong><small>${formatPercent(currentMetrics.total?task.total/currentMetrics.total*100:0)}</small></span></button>`).join(''):`<div class="empty-state">${t('adjustFilters')}</div>`;$('top-expensive-tasks').querySelectorAll('[data-task]').forEach(button=>button.addEventListener('click',()=>openTask(button.dataset.task)));}
function renderTaskTable(tasks){const query=state.search.trim().toLowerCase(),matching=tasks.filter(task=>`${task.task} ${task.project} ${task.model}`.toLowerCase().includes(query)).sort((a,b)=>state.descending?b.total-a.total:a.total-b.total),pageSize=6,pages=Math.max(1,Math.ceil(matching.length/pageSize));state.page=Math.min(state.page,pages-1);const visible=matching.slice(state.page*pageSize,state.page*pageSize+pageSize);$('task-count').textContent=`/ ${matching.length}`;$('task-table-body').innerHTML=visible.length?visible.map(task=>{const driver=currentDrivers.get(task.taskId);return `<tr><td><button class="task-link" data-task="${task.taskId}">${escapeHTML(task.task)}</button><span class="task-project">${escapeHTML(task.project)} · ${t('turnCount',task.count)}</span></td><td><span class="model-tag">${escapeHTML(shortModel(task.model))}</span></td><td><span class="rank-value">${formatTokens(task.total)}</span></td><td>${formatTokens(task.tokensPerTurn)}</td><td class="speed-value">${task.speed?`${task.speed.toFixed(1)}/s`:t('notAvailable')}</td><td>${formatPercent(task.cacheRate*100,0)}</td><td><span class="driver-tag">${escapeHTML(driver.labels[0])}</span></td></tr>`;}).join(''):`<tr><td colspan="7" class="table-empty">${t('noData')}</td></tr>`;$('table-status').textContent=t('showing',matching.length?state.page*pageSize+1:0,Math.min((state.page+1)*pageSize,matching.length),matching.length);$('page-number').textContent=`${state.page+1} / ${pages}`;$('previous-page').disabled=state.page===0;$('next-page').disabled=state.page>=pages-1;$('task-table-body').querySelectorAll('[data-task]').forEach(button=>button.addEventListener('click',()=>openTask(button.dataset.task)));}
