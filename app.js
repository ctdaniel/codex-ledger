...
function buildDemoRecords(){
  ...
  for(let cursor=new Date(fixtureStart);cursor<=fixtureEnd;cursor.setUTCDate(cursor.getUTCDate()+1)){
    ...
    DEMO_TASKS.forEach((task,taskIndex)=>{
      ...
      const turns=Math.max(3,Math.round((3+random()*8)*scale));
      const sessionId=`${task.id}-${date}`;
      for(let turn=0;turn<turns;turn+=1){
        const growthBase=task.id==='task-9'?.23:task.id==='task-12'?.18:.055+.045*random();
        const contextGrowth=1+turn*growthBase;
        const input=Math.round((22000+random()*78000)*task.weight*(.82+random()*.36)*contextGrowth);
        ...
        const hour=9+Math.floor(turn/2),minute=turn%2?35:10,time=`${String(hour).padStart(2,'0')}:${minute}`;
        output.push({
          id:`demo-${date}-${taskIndex}-${turn}`,timestamp:`${date}T${time}:00Z`,date,time,
          taskId:sessionId,turnId:`${sessionId}-turn-${turn+1}`,task:task.name,project:task.project,model:task.model,
...
function normalizeRecord(row,index){
  ...
  const date=String(row.date||'').slice(0,10),time=String(row.time||'00:00').slice(0,5),taskId=String(row.taskId||row.sessionId||`task-${index}`);
  return {
    id:String(row.id||`record-${index}`),timestamp:String(row.timestamp||`${date}T${time}:00Z`),date,time,
    taskId,turnId:String(row.turnId||`${taskId}-turn-${index}`),
...
const average = values => values.length ? values.reduce((total,value)=>total+(Number(value)||0),0)/values.length : 0;
const recordEpoch = row => { const parsed=Date.parse(row.timestamp||`${row.date}T${row.time||'00:00'}:00Z`); return Number.isFinite(parsed)?parsed:0; };
function aggregateTaskTurns(rows){
  const turns=new Map();
  [...rows].sort((a,b)=>recordEpoch(a)-recordEpoch(b)||String(a.id).localeCompare(String(b.id))).forEach(row=>{
    const turnId=row.turnId||row.id;
    if(!turns.has(turnId))turns.set(turnId,{turnId,input:0,cached:0,output:0,total:0,durationSeconds:0,timestamp:row.timestamp,date:row.date,time:row.time,model:row.model});
    const turn=turns.get(turnId);
    // A user turn may contain multiple model requests. Max input is the closest
    // observable proxy for context size; token totals still sum every request.
    turn.input=Math.max(turn.input,row.input||0);
    turn.cached=Math.max(turn.cached,row.cached||0);
    turn.output+=(row.output||0);
    turn.total+=(row.total||0);
    turn.durationSeconds+=(row.durationSeconds||0);
    if(recordEpoch(row)>=recordEpoch(turn)){turn.timestamp=row.timestamp;turn.date=row.date;turn.time=row.time;turn.model=row.model;}
  });
  return [...turns.values()].sort((a,b)=>recordEpoch(a)-recordEpoch(b)||String(a.turnId).localeCompare(String(b.turnId))).map((turn,index)=>({...turn,index:index+1}));
}
function aggregateTasks(rows) {
  const grouped=new Map();
  rows.forEach(row=>{
    if(!grouped.has(row.taskId)) grouped.set(row.taskId,{taskId:row.taskId,task:row.task,project:row.project,model:row.model,input:0,cached:0,output:0,reasoning:0,total:0,durationSeconds:0,measuredOutput:0,credits:0,rows:[]});
    const item=grouped.get(row.taskId); ['input','cached','output','reasoning','total'].forEach(key=>item[key]+=row[key]); item.credits+=creditsForRecord(row); item.rows.push(row);
    if(row.durationSeconds>0){item.durationSeconds+=row.durationSeconds;item.measuredOutput+=(row.speedOutput||row.output);}
  });
  return [...grouped.values()].map(item=>{
    const turns=aggregateTaskTurns(item.rows),windowSize=Math.min(5,turns.length),first=turns.slice(0,windowSize),last=turns.slice(-windowSize);
    const firstAvgInput=average(first.map(turn=>turn.input)),lastAvgInput=average(last.map(turn=>turn.input));
    const contextGrowth=turns.length>=3&&firstAvgInput>0?lastAvgInput/firstAvgInput:null;
    const peakTurn=turns.reduce((peak,turn)=>!peak||turn.total>peak.total?turn:peak,null);
    const startedAt=turns[0]?.timestamp||null,endedAt=turns.at(-1)?.timestamp||null,startMs=startedAt?Date.parse(startedAt):NaN,endMs=endedAt?Date.parse(endedAt):NaN;
    const spanSeconds=Number.isFinite(startMs)&&Number.isFinite(endMs)?Math.max(0,(endMs-startMs)/1000):0;
    return {...item,rows:undefined,turns,count:turns.length,eventCount:item.rows.length,speed:item.durationSeconds?item.measuredOutput/item.durationSeconds:null,cacheRate:item.input?item.cached/item.input:0,tokensPerTurn:turns.length?item.total/turns.length:null,firstAvgInput,lastAvgInput,contextGrowth,peakTurn,startedAt,endedAt,spanSeconds,recentTokensPerTurn:average(last.map(turn=>turn.total))};
  });
}
function aggregateModels(rows) {
  const grouped=new Map();
  rows.forEach(row=>{
    if(!grouped.has(row.model)) grouped.set(row.model,{model:row.model,input:0,cached:0,output:0,total:0,durationSeconds:0,measuredOutput:0,credits:0,calls:0,taskIds:new Set(),turnIds:new Set()});
    const item=grouped.get(row.model); ['input','cached','output','total'].forEach(key=>item[key]+=row[key]); item.calls+=1; item.credits+=creditsForRecord(row); item.taskIds.add(row.taskId); item.turnIds.add(`${row.taskId}:${row.turnId||row.id}`); item.durationSeconds+=row.durationSeconds; item.measuredOutput+=row.durationSeconds>0?(row.speedOutput||row.output):0;
  });
  return [...grouped.values()].map(item=>({...item,tasks:item.taskIds.size,turns:item.turnIds.size,speed:item.durationSeconds?item.measuredOutput/item.durationSeconds:null,cacheRate:item.input?item.cached/item.input:0,tokensPerTask:item.taskIds.size?item.total/item.taskIds.size:null,tokensPerTurn:item.turnIds.size?item.total/item.turnIds.size:null}));
}
const percentile = (values,p) => { const clean=values.filter(Number.isFinite); if(!clean.length)return 0; const sorted=[...clean].sort((a,b)=>a-b); return sorted[Math.min(sorted.length-1,Math.floor((sorted.length-1)*p))]; };
function sessionBaselines(tasks){
  return {
    medianTokensPerTurn:percentile(tasks.map(task=>task.tokensPerTurn),.5),
    p90TokensPerTurn:percentile(tasks.map(task=>task.tokensPerTurn),.9),
    medianGrowth:percentile(tasks.map(task=>task.contextGrowth).filter(Number.isFinite),.5)||1,
    p90Growth:percentile(tasks.map(task=>task.contextGrowth).filter(Number.isFinite),.9)||1,
    medianTurns:percentile(tasks.map(task=>task.count),.5),
    p90Turns:percentile(tasks.map(task=>task.count),.9),
    medianCache:percentile(tasks.map(task=>task.cacheRate).filter(Number.isFinite),.5)
  };
}
function classifySession(task,baseline){
  if(task.count<3||!Number.isFinite(task.contextGrowth))return 'limited';
  const highGrowth=task.contextGrowth>=Math.max(3,baseline.medianGrowth*1.8);
  const elevatedGrowth=task.contextGrowth>=Math.max(1.7,baseline.medianGrowth*1.25);
  const highTurn=task.tokensPerTurn>=Math.max(baseline.p90TokensPerTurn,baseline.medianTokensPerTurn*2);
  const long=task.count>=Math.max(30,baseline.p90Turns||30);
  if((highGrowth&&(highTurn||long))||task.contextGrowth>=5)return 'heavy';
  if(highGrowth||elevatedGrowth)return 'growing';
  if(highTurn||long)return 'heavy';
  return 'stable';
}
function calculateMetrics(rows,bounds) {
  const tasks=aggregateTasks(rows),baseline=sessionBaselines(tasks); tasks.forEach(task=>task.health=classifySession(task,baseline));
  const input=sum(rows,'input'),cached=sum(rows,'cached'),total=sum(rows,'total'),days=bounds[0]<=bounds[1]?dateRange(...bounds).length:0,turns=tasks.reduce((n,task)=>n+task.count,0);
  return {rows,tasks,models:aggregateModels(rows),sessionBaseline:baseline,total,input,cached,output:sum(rows,'output'),uncached:input-cached,cacheRate:input?cached/input:null,burnRate:days?total/days:null,tokensPerTask:tasks.length?total/tasks.length:null,tokensPerTurn:turns?total/turns:null,taskCount:tasks.length,turns,calls:rows.length,days};
}
function calculateTaskDrivers(tasks) {
  const driverLabel=label=>state.language==='zh'?({
    'Large Task':'高消耗任务','Long Session':'长 Session','Low Cache':'低缓存命中','Output Heavy':'输出偏高','High Token / Turn':'单轮 Token 偏高','High Context Growth':'Context 增长高','Peak Turn':'峰值轮次','Normal':'稳定'
  }[label]||label):label;
  const result=new Map(),baseline=sessionBaselines(tasks),p90Total=percentile(tasks.map(t=>t.total),.9),p90Output=percentile(tasks.map(t=>t.total?t.output/t.total:0),.9);
  tasks.forEach(task=>{
    ...
    if(task.count>=Math.max(30,baseline.p90Turns||30)){labels.push(driverLabel('Long Session'));reasons.push(t('driverLongSession',task.count));}
    if(Number.isFinite(task.contextGrowth)&&task.contextGrowth>=Math.max(2.5,baseline.medianGrowth*1.5)){labels.push(driverLabel('High Context Growth'));reasons.push(t('driverContextGrowth',task.contextGrowth.toFixed(1)));}
    if(task.cacheRate<Math.min(.65,baseline.medianCache*.8)){labels.push(driverLabel('Low Cache'));reasons.push(t('driverCacheCompare',formatPercent(task.cacheRate*100),formatPercent(baseline.medianCache*100)));}
    if(task.total&&task.output/task.total>=p90Output&&tasks.length>2){labels.push(driverLabel('Output Heavy'));reasons.push(t('driverOutput'));}
    if(task.tokensPerTurn>=Math.max(baseline.p90TokensPerTurn,baseline.medianTokensPerTurn*2)&&tasks.length>2){labels.push(driverLabel('High Token / Turn'));reasons.push(t('driverTurn',(task.tokensPerTurn/Math.max(baseline.medianTokensPerTurn,1)).toFixed(1)));}
    if(task.peakTurn&&task.tokensPerTurn&&task.peakTurn.total>=task.tokensPerTurn*2.2){labels.push(driverLabel('Peak Turn'));reasons.push(t('driverPeakTurn',task.peakTurn.index,formatTokens(task.peakTurn.total)));}
    result.set(task.taskId,{labels:labels.length?labels.slice(0,4):[driverLabel('Normal')],reasons:reasons.length?reasons.slice(0,4):[t('driverNormal')]});
  });
  return result;
}
...
function applyStaticLanguage(){
  ...
  const bindings=[
    ...
    ['#insights-title','Usage insights','使用洞察'],['.insights-panel .section-head p','Generated from transparent rules using the current filters. Up to four insights.','由当前筛选数据按透明规则生成，最多显示 4 条。'],['.insights-panel .scope-label','Calculated · Current period','Calculated · 当前周期'],['#session-health-title','Session health','Session 健康度'],['#session-health-copy','Transparent status from context growth, turns, and tokens / turn.','基于 Context 增长、轮数与单轮 Token 的透明状态判断。'],['#session-health-scope','Calculated · Your baseline','Calculated · 你的历史基线'],['#context-watch-title','Context growth watch','Context 增长观察'],['#context-watch-copy','First 5 vs last 5 turns. Click a session for detail.','对比前 5 轮与后 5 轮；点击 Session 查看明细。'],
    ...
    ['th:nth-child(1)','Task / Project','任务 / 项目'],['th:nth-child(2)','Model','模型'],['th:nth-child(4)','Tokens / Turn','单轮 Token'],['th:nth-child(5)','Context growth','Context 增长'],['th:nth-child(6)','Cache','缓存命中'],['th:nth-child(7)','Health','状态'],['#previous-page','Previous','上一页'],['#next-page','Next','下一页'],
  ...
  const ariaBindings=[
    ...
    ['.session-grid','Session health and context growth','Session 健康与 Context 增长'],['.diagnostics-grid','Cross-analysis and task outliers','交叉分析与异常任务'],['#outlier-chart','Task token volume and cache-hit scatter plot','任务 Token 总量与缓存命中率散点图'],['.attribution-grid','Usage attribution','消耗归因'],['.tasks-panel','Task details','任务明细']
  ];
...
function renderSummary(){
  const totalChange=changeRate(currentMetrics.total,previousMetrics.total),burnChange=changeRate(currentMetrics.burnRate||0,previousMetrics.burnRate||0),taskChange=changeRate(currentMetrics.tokensPerTask||0,previousMetrics.tokensPerTask||0),turnChange=changeRate(currentMetrics.tokensPerTurn||0,previousMetrics.tokensPerTurn||0),cacheChange=changeRate(currentMetrics.cacheRate||0,previousMetrics.cacheRate||0);
  ...
  $('tokens-task').textContent=currentMetrics.tokensPerTask?formatTokens(currentMetrics.tokensPerTask):'—'; $('tokens-task-detail').innerHTML=`${formatChange(taskChange)} · ${t('tasksTurns',currentMetrics.taskCount,currentMetrics.turns)}`;
  $('tokens-turn').textContent=currentMetrics.tokensPerTurn?formatTokens(currentMetrics.tokensPerTurn):'—'; $('tokens-turn-detail').innerHTML=`${formatChange(turnChange)} · ${formatNumber(currentMetrics.turns)} ${state.language==='zh'?'轮':'turns'}`;
...
function calculateInsights(){
  const eyebrow=state.language==='zh'?{quota:'额度 / 节奏',context:'Session / Context',turn:'单轮效率',growth:'项目增长',task:'高消耗任务',model:'模型集中度',cache:'缓存'}:{quota:'Quota / Pacing',context:'Session / Context',turn:'Per-turn usage',growth:'Project Growth',task:'Expensive Task',model:'Model Concentration',cache:'Cache'};
  const insights=[];
  ...
  const growthSessions=currentMetrics.tasks.filter(task=>Number.isFinite(task.contextGrowth)&&task.contextGrowth>=2.5).sort((a,b)=>b.contextGrowth-a.contextGrowth);
  if(growthSessions.length){insights.push({tone:'orange',eyebrow:eyebrow.context,title:t('contextInsightTitle'),body:t('contextInsightBody',growthSessions.length,growthSessions[0].contextGrowth.toFixed(1))});}
  const turnChange=changeRate(currentMetrics.tokensPerTurn||0,previousMetrics.tokensPerTurn||0);
  if(Number.isFinite(turnChange)&&Math.abs(turnChange)>.18){const up=turnChange>0;insights.push({tone:up?'blue':'green',eyebrow:eyebrow.turn,title:t(up?'turnRateUp':'turnRateDown'),body:t('turnRateBody',Math.abs(turnChange*100).toFixed(1),formatTokens(currentMetrics.tokensPerTurn))});}
...
function healthLabel(status){return t(status==='stable'?'sessionStable':status==='growing'?'sessionGrowing':status==='heavy'?'sessionHeavy':'sessionLimited');}
function renderSessionHealth(){
  const tasks=currentMetrics.tasks,baseline=currentMetrics.sessionBaseline||sessionBaselines(tasks),counts={stable:0,growing:0,heavy:0,limited:0};tasks.forEach(task=>counts[task.health||'limited']+=1);
  const stats=[['stable',counts.stable],['growing',counts.growing],['heavy',counts.heavy],['limited',counts.limited]];
  $('session-health-summary').innerHTML=stats.map(([key,value])=>`<div class="health-stat ${key}"><span>${escapeHTML(healthLabel(key))}</span><strong>${value}</strong></div>`).join('');
  const baselineRows=state.language==='zh'?[
    ['你的单轮中位数',formatTokens(baseline.medianTokensPerTurn)],['你的 Context 增长中位数',`${(baseline.medianGrowth||1).toFixed(1)}×`],['你的 Session 轮数中位数',formatNumber(baseline.medianTurns)]
  ]:[['Your median tokens / turn',formatTokens(baseline.medianTokensPerTurn)],['Your median context growth',`${(baseline.medianGrowth||1).toFixed(1)}×`],['Your median session turns',formatNumber(baseline.medianTurns)]];
  $('session-baseline-summary').innerHTML=baselineRows.map(([label,value])=>`<div class="baseline-row"><span>${label}</span><strong>${value}</strong></div>`).join('');
  const watch=[...tasks].filter(task=>Number.isFinite(task.contextGrowth)).sort((a,b)=>b.contextGrowth-a.contextGrowth).slice(0,5),maxGrowth=Math.max(...watch.map(task=>task.contextGrowth),1);
  $('context-watch-list').innerHTML=watch.length?watch.map(task=>`<button class="context-watch-item" data-task="${task.taskId}" title="${escapeHTML(task.task)}"><span class="context-watch-copy"><strong>${escapeHTML(task.task)}</strong><small>${escapeHTML(task.project)} · ${escapeHTML(shortModel(task.model))} · ${task.count} ${state.language==='zh'?'轮':'turns'}</small><span class="context-growth-meter"><i style="--growth:${Math.min(1,task.contextGrowth/maxGrowth)}"></i></span></span><span class="context-watch-value"><strong>${task.contextGrowth.toFixed(1)}×</strong><small class="health-chip ${task.health}">${escapeHTML(healthLabel(task.health))}</small></span></button>`).join(''):`<div class="empty-state">${state.language==='zh'?'当前筛选范围暂无足够的 Session 数据。':'Not enough session data in the current filters.'}</div>`;
  $('context-watch-list').querySelectorAll('[data-task]').forEach(button=>button.addEventListener('click',()=>openTask(button.dataset.task)));
}
...
function rankRows(items,type,maxValue,grandTotal,previousMap){
  ...
  return items.map((item,index)=>{
    const isBurn=type==='burn',isSpeed=type==='speed'; const value=isBurn?(item.turns?item.credits/item.turns:0):isSpeed?item.speed:item.total; const width=maxValue?value/maxValue:0; const previous=previousMap.get(item.model); const change=changeRate(item.total,previous?previous.total:0); let valueText,detail;
    ...
    else if(isSpeed){valueText=`${item.speed.toFixed(1)}<small> token/s</small>`;detail=t('turnsOutput',item.turns,formatTokens(item.output));}
...
function renderModels(){
  ...
  const left=state.leftMetric==='burn'?byBurn:bySpeed,maxLeft=Math.max(...left.map(item=>state.leftMetric==='burn'?(item.turns?item.credits/item.turns:0):item.speed),1);
...
function renderTaskTable(tasks){const query=state.search.trim().toLowerCase(),matching=tasks.filter(task=>`${task.task} ${task.project} ${task.model}`.toLowerCase().includes(query)).sort((a,b)=>state.descending?b.total-a.total:a.total-b.total),pageSize=8,pages=Math.max(1,Math.ceil(matching.length/pageSize));state.page=Math.min(state.page,pages-1);const visible=matching.slice(state.page*pageSize,state.page*pageSize+pageSize);$('task-count').textContent=`/ ${matching.length}`;$('task-table-body').innerHTML=visible.length?visible.map(task=>{const driver=currentDrivers.get(task.taskId)||{labels:['Normal']},growth=Number.isFinite(task.contextGrowth)?`${task.contextGrowth.toFixed(1)}×`:'—';return `<tr><td><button class="task-link" data-task="${task.taskId}" title="${escapeHTML(task.task)}">${escapeHTML(task.task)}</button><span class="task-project" title="${escapeHTML(task.project)}">${escapeHTML(task.project)} · ${t('turnCount',task.count)}</span></td><td><span class="model-tag" title="${escapeHTML(task.model)}">${escapeHTML(shortModel(task.model))}</span></td><td class="numeric-cell"><span class="rank-value">${formatTokens(task.total)}</span></td><td class="numeric-cell">${formatTokens(task.tokensPerTurn)}</td><td class="numeric-cell">${growth}</td><td class="numeric-cell">${formatPercent(task.cacheRate*100,0)}</td><td><span class="health-chip ${task.health}" title="${escapeHTML(driver.labels.join(', '))}">${escapeHTML(healthLabel(task.health))}</span></td></tr>`;}).join(''):`<tr><td colspan="7" class="table-empty">${t('noData')}</td></tr>`;$('table-status').textContent=t('showing',matching.length?state.page*pageSize+1:0,Math.min((state.page+1)*pageSize,matching.length),matching.length);$('page-number').textContent=`${state.page+1} / ${pages}`;$('previous-page').disabled=state.page===0;$('next-page').disabled=state.page>=pages-1;$('task-table-body').querySelectorAll('[data-task]').forEach(button=>button.addEventListener('click',()=>openTask(button.dataset.task)));}
function contextGrowthChart(task){
  const turns=task.turns||[]; if(turns.length<2)return `<div class="empty-state">${state.language==='zh'?'至少需要 2 轮数据才能绘制 Context 曲线。':'At least 2 turns are needed for a context curve.'}</div>`;
  const width=430,height=132,left=34,right=416,top=12,bottom=105,max=Math.max(...turns.map(turn=>turn.input),1),span=Math.max(1,turns.length-1);
  const points=turns.map((turn,index)=>({x:left+(right-left)*(index/span),y:bottom-(turn.input/max)*(bottom-top),turn}));
  const line=points.map((point,index)=>`${index?'L':'M'}${point.x.toFixed(1)} ${point.y.toFixed(1)}`).join(' '),area=`M${left} ${bottom} ${line.slice(1)} L${right} ${bottom} Z`,peak=task.peakTurn,peakPoint=peak?points[Math.max(0,Math.min(points.length-1,peak.index-1))]:null;
  return `<div class="context-chart"><svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Context input growth by turn"><line x1="${left}" x2="${right}" y1="${bottom}" y2="${bottom}" stroke="var(--hairline)"/><line x1="${left}" x2="${right}" y1="${top}" y2="${top}" stroke="var(--hairline)" stroke-dasharray="3 4"/><path class="context-area" d="${area}"/><path class="context-line" d="${line}"/>${points.map((point,index)=>`<circle class="context-dot ${peakPoint===point?'peak':''}" cx="${point.x}" cy="${point.y}" r="${peakPoint===point?3.5:2.1}"/>`).join('')}<text class="context-axis" x="${left}" y="124">1</text><text class="context-axis" x="${right}" y="124" text-anchor="end">${turns.length}</text><text class="context-axis" x="${left}" y="10">${formatTokens(max)}</text></svg></div>`;
}
function openTask(taskId){
  const rows=filteredRecords.filter(r=>r.taskId===taskId),task=aggregateTasks(rows)[0];if(!task)return;task.health=classifySession(task,currentMetrics.sessionBaseline||sessionBaselines(currentMetrics.tasks));lastFocusedElement=document.activeElement;
  const zh=state.language==='zh',driver=currentDrivers.get(taskId)||{labels:['Normal'],reasons:[t('driverNormal')]},ui=zh?{detail:'Session 详情',total:'Token 总量',turns:'轮次',perTurn:'单轮 Token',duration:'生成时长',span:'观测跨度',cache:'缓存命中',speed:'输出速度',health:'Session 状态',context:'Context 增长',first:'前 5 轮平均输入',last:'后 5 轮平均输入',growth:'增长倍率',peak:'峰值轮次',driver:'消耗特征',breakdown:'Token 构成',input:'输入',cached:'缓存输入',uncached:'非缓存输入',output:'输出',calls:'逐轮明细',note:hasCreditRate(task.model)?`Estimated Credits：${task.credits.toFixed(1)} cr。仅用于模型费率权重比较，不代表账户额度百分比。`:'Estimated Credits：当前模型无内置费率映射，因此不估算。'}:{detail:'Session detail',total:'Total tokens',turns:'Turns',perTurn:'Tokens / turn',duration:'Generation time',span:'Observed span',cache:'Cache hit',speed:'Output speed',health:'Session health',context:'Context growth',first:'First 5 avg input',last:'Last 5 avg input',growth:'Growth ratio',peak:'Peak turn',driver:'Consumption drivers',breakdown:'Token breakdown',input:'Input',cached:'Cached input',uncached:'Non-cached input',output:'Output',calls:'Turn detail',note:hasCreditRate(task.model)?`Estimated credits: ${task.credits.toFixed(1)} cr. This is only a model-rate weighting and is not an account quota percentage.`:'Estimated credits are unavailable because this model has no built-in rate mapping.'};
  const growth=Number.isFinite(task.contextGrowth)?`${task.contextGrowth.toFixed(1)}×`:'—',peak=task.peakTurn?`${zh?'第':'Turn '}${task.peakTurn.index}${zh?' 轮':''} · ${formatTokens(task.peakTurn.total)}`:'—';
  $('drawer-content').innerHTML=`<span class="drawer-label">${ui.detail} · Observed / Calculated</span><h2 id="drawer-title">${escapeHTML(task.task)}</h2><p class="drawer-subtitle">${escapeHTML(task.project)} · ${escapeHTML(task.model)} · <span class="health-chip ${task.health}">${escapeHTML(healthLabel(task.health))}</span></p><div class="drawer-metrics"><div class="drawer-metric"><span>${ui.total}</span><strong>${formatTokens(task.total)}</strong></div><div class="drawer-metric"><span>${ui.turns}</span><strong>${task.count}</strong></div><div class="drawer-metric"><span>${ui.perTurn}</span><strong>${formatTokens(task.tokensPerTurn)}</strong></div><div class="drawer-metric"><span>${ui.duration}</span><strong>${formatDuration(task.durationSeconds)}</strong></div><div class="drawer-metric"><span>${ui.cache}</span><strong>${formatPercent(task.cacheRate*100)}</strong></div><div class="drawer-metric"><span>${ui.span}</span><strong>${task.spanSeconds?formatDuration(task.spanSeconds):'—'}</strong></div></div><h3>${ui.context}</h3><div class="context-metrics"><div class="context-metric"><span>${ui.first}</span><strong>${formatTokens(task.firstAvgInput)}</strong></div><div class="context-metric"><span>${ui.last}</span><strong>${formatTokens(task.lastAvgInput)}</strong></div><div class="context-metric"><span>${ui.growth}</span><strong>${growth}</strong></div><div class="context-metric"><span>${ui.peak}</span><strong>${peak}</strong></div><div class="context-metric"><span>${ui.speed}</span><strong>${task.speed?`${task.speed.toFixed(1)} t/s`:'—'}</strong></div><div class="context-metric"><span>${ui.health}</span><strong>${escapeHTML(healthLabel(task.health))}</strong></div></div>${contextGrowthChart(task)}<h3>${ui.driver}</h3><div class="driver-list">${driver.labels.map(label=>`<span class="driver-tag">${escapeHTML(label)}</span>`).join('')}</div><ul class="driver-reasons">${driver.reasons.map(reason=>`<li>${escapeHTML(reason)}</li>`).join('')}</ul><h3>${ui.breakdown}</h3><div class="breakdown-list"><span>${ui.input} <b>${formatTokens(task.input)}</b></span><span>${ui.cached} <b>${formatTokens(task.cached)}</b></span><span>${ui.uncached} <b>${formatTokens(task.input-task.cached)}</b></span><span>${ui.output} <b>${formatTokens(task.output)}</b></span></div><p class="drawer-note">${ui.note}</p><h3>${ui.calls}</h3>${task.turns.slice().reverse().slice(0,30).map(turn=>`<div class="call-row"><span>${turn.date.slice(5)} ${turn.time} · #${turn.index}</span><strong>${formatTokens(turn.total)} tok</strong><em>${formatTokens(turn.input)} in</em></div>`).join('')}`;
  $('drawer-backdrop').classList.add('open');document.body.style.overflow='hidden';$('drawer-close').focus();
}
